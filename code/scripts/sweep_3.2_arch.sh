...
# ═══════════════════════════════════════════════════════════════════════════════
#   Introduce all three modifications simultaneously:
#     QK Norm + Attention Gate + Value Embedding
#
#   Train this enhanced architecture at 5 model sizes, then plot on the same
#   Loss vs Non-Embedding Parameters graph together with the §3.1 baseline.
# ═══════════════════════════════════════════════════════════════════════════════
...
echo ""
echo "========== 3.2 Architecture sweep done =========="
echo ""
echo "  Compare with §3.1 baseline on the same graph using analyze_partb.py"
echo ""
printf "%-22s %14s %14s %14s\n" "Tag" "Non-Emb Params" "Best Valid PPL" "Best Valid Loss"
printf "%-22s %14s %14s %14s\n" "---" "--------------" "--------------" "--------------"
for dim in 64 96 128 160 192; do
    f="$RESULT_DIR/results_arch_all3_d${dim}.json"
    if [ -f "$f" ]; then
        python3 -c "
import json
d = json.load(open('$f'))
print(f\"arch_all3_d${dim:<3s}  {d['config']['non_embedding_params']:>14,d}  {d['best_valid_ppl']:>14.2f}  {d['valid_epoch_loss'][d['best_epoch']-1]:>14.4f}\")
"
    else
        printf "arch_all3_d%-3s  %14s\n" "${dim}" "(not found)"
    fi
done
